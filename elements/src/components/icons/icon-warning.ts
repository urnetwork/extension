import { html, LitElement } from "lit";
import { customElement } from "lit/decorators.js";
import { iconWarningStyles } from "./icon-warning.styles";

// from Streamline Icons - https://www.streamlinehq.com/icons/pixel?search=warning&icon=ico_FgN7Lo8RjrSWlCmE
@customElement("ur-icon-warning")
export class IconWarning extends LitElement {
	static styles = iconWarningStyles;

	render() {
		return html`
			<svg
				xmlns="http://www.w3.org/2000/svg"
				viewBox="0 0 32 32"
				id="Interface-Essential-Alert-Triangle-1--Streamline-Pixel"
			>
				<desc>
					Interface Essential Alert Triangle 1 Streamline Icon:
					https://streamlinehq.com
				</desc>
				<title>interface-essential-alert-triangle-1</title>
				<g>
					<path
						d="M30.48 27.43H32v3.05h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M28.95 24.38h1.53v3.05h-1.53Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M1.52 30.48h28.96V32H1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M27.43 21.33h1.52v3.05h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M25.9 18.29h1.53v3.04H25.9Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M24.38 15.24h1.52v3.05h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M22.86 12.19h1.52v3.05h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M21.33 9.14h1.53v3.05h-1.53Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M19.81 6.09h1.52v3.05h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M18.29 3.05h1.52v3.04h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="m18.29 21.33 -4.58 0 0 1.53 -1.52 0 0 4.57 1.52 0 0 1.52 4.58 0 0 -1.52 1.52 0 0 -4.57 -1.52 0 0 -1.53z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M18.29 9.14h-4.58v1.53h-1.52v6.09h1.52v3.05h4.58v-3.05h1.52v-6.09h-1.52Zm0 6.1h-1.53v-3.05h-1.52v-1.52h1.52v1.52h1.53Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M16.76 1.52h1.53v1.53h-1.53Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M15.24 0h1.52v1.52h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M13.71 1.52h1.53v1.53h-1.53Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M12.19 3.05h1.52v3.04h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M10.67 6.09h1.52v3.05h-1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M9.14 9.14h1.53v3.05H9.14Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M7.62 12.19h1.52v3.05H7.62Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M6.09 15.24h1.53v3.05H6.09Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M4.57 18.29h1.52v3.04H4.57Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M3.05 21.33h1.52v3.05H3.05Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M1.52 24.38h1.53v3.05H1.52Z"
						fill="currentColor"
						stroke-width="1"
					></path>
					<path
						d="M0 27.43h1.52v3.05H0Z"
						fill="currentColor"
						stroke-width="1"
					></path>
				</g>
			</svg>
		`;
	}
}

declare global {
	interface HTMLElementTagNameMap {
		"ur-icon-warning": IconWarning;
	}
}
